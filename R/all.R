...
muhaz <- function(times, delta, subset, min.time, max.time, bw.grid, bw.pilot,
                  ...
                  n.est.grid=101, kern="epanechnikov",w)
{
    ...
    #In case w is missing OR there is no confounders a non-weighted Nelson Aalen is computed 
    if (missing(w)){
    ...
    endl <- startz
    endr <- endz
    # the Nelson-Aalen calculation-this is the extra code 
    fit.1 <- survfit(Surv(times, delta) ~ 1,weights =w)
    fit.1.t<-summary(fit.1,time=times)
    naes<-c(fit.1.t$cumhaz[1],diff(fit.1.t$cumhaz))
    nw<-sum(w)
    ...
    pin.common <- list(times=times, delta=delta,
                       ...
                       method=method, b.cor=b.cor, kernel.type=kern,w=w,
                       naes=naes,nw=nw)
    ...
    if (method < 3) {
        ...
        ans <- .Fortran("newhad",
                        ...
                        imsemin = double(1),
                        globlb = double(1),
                        globlmse = double(gridb),as.double(w),
                        as.double(naes),as.double(nw),
			PACKAGE = "muhaz")
